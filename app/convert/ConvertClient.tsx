...
export default function ConvertClient() {
  ...
  async function handleStartSpotifySearch(limit?: number) {
    setStage("searching");
    setProgress(0);
    setItems([]);
    setFilter("all");
    ...
    const itemsToSearch = limit ? ytItems.slice(0, limit) : ytItems;
    const totalToSearch = itemsToSearch.length;
    setSearchTotal(totalToSearch);
    const matched: MatchedItem[] = [];
    for (let i = 0; i < totalToSearch; i++) {
      const yt = itemsToSearch[i];
      setProgress(i + 1);
      ...
      if (spRes.status === 429) {
        ...
        setSpotifyError(`Rate limit Spotify — czekam ${waitSec}s i ponawiam... (${i + 1}/${itemsToSearch.length})`);
      ...
      if (spRes.ok) {
        ...
        for (let j = i; j < itemsToSearch.length; j++) {
          matched.push({ ytTitle: itemsToSearch[j].title, ytThumbnail: itemsToSearch[j].thumbnail, videoId: itemsToSearch[j].videoId, spotifyTracks: [], selectedTrack: null });
        }
        setItems(matched);
        setStage("results");
        return;
  ...
  async function handleExport() {
    ...
    try {
      ...
      setError("Blad sieci podczas eksportu — sprobuj ponownie");
      setStage("export-setup");
    }
  }
      ...
      <div>
        ...
        {ytItems.length > 0 && (
          <div className="space-y-2">
            ...
            {ytItems.length > 3 && (
              <button
                onClick={() => handleStartSpotifySearch(3)}
                className="w-full text-sm py-2 bg-gray-800 hover:bg-gray-700 text-gray-400 rounded-xl border border-gray-700 transition-colors cursor-pointer"
              >
                Testuj — szukaj tylko pierwsze 3 utwory
              </button>
            )}
          </div>
        )}
  ...
  if (stage === "searching") {
    const pct = searchTotal > 0 ? Math.round((progress / searchTotal) * 100) : 0;
    return (
      <div>
        <StepIndicator stage={stage} />
        <div className="bg-gray-900 rounded-2xl p-8 border border-gray-800 text-center">
          {spotifyError ? (
            ...
            <div className="mb-4 text-gray-300">
              Wyszukiwanie na Spotify: {progress} / {searchTotal}
            </div>
          )}
